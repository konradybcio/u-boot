...
#define GPLL0_OUT_AUX2_PWRGRP35_CKGEN_ACGCR	(0x0F70)
...
static void clk_enable_peripherals(phys_addr_t reg_addr)
{
	setbits_le32(reg_addr, GCC_CLOBAL_PERIPHERALS_EN);
}

#define CGCR_BRANCH_ENABLE_BIT	BIT(0)
#define CGCR_CLK_ON_CHECK_BIT	BIT(31)
/* Enable clock controlled by ACGCR */
void clk_enable_cgcr(phys_addr_t cgcr)
{
	setbits_le32(cgcr, CGCR_BRANCH_ENABLE_BIT);
	
	while (((readl(cgcr) & CGCR_CLK_ON_CHECK_BIT) >> 31) == 0)
		;
}
